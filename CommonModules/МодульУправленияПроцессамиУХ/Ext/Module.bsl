...
#КонецВставки
	Если ТаблицаОтветственных.Количество() > 0 Тогда
		...
		ЕстьУточнениеЗамещения = Ложь;
		Если ДобавлятьЗаместителейВход Тогда
			ТЗЗамещаемых = ТаблицаЗамещаемых(Пользователь, ЕстьУточнениеЗамещения);
			Если ТЗЗамещаемых.Количество() > 0 Тогда
				Если ЕстьУточнениеЗамещения Тогда
					СписокЗамещаемых = ЗамещаемыеПоОбъектуСогласования(
						Пользователь,
						ОбъектСогласования,
						ТаблицаРасшифровка,
						ТЗЗамещаемых,
						ЭкземплярПроцесса.ТипКлючевогоОбъекта);
					Для Каждого Стр Из СписокЗамещаемых Цикл
						СписокУтверждающих.Добавить(Стр.Значение);
					КонецЦикла;
				Иначе
					// todo просто добавить утвержающих в список.
