...
&НаКлиенте
Процедура ДоговорУчетаПриИзменении(Элемент)
	ДоговорУчетаПриИзмененииНаСервере();
КонецПроцедуры  

&НаСервере
Процедура ДоговорУчетаПриИзмененииНаСервере()
	...
	Иначе 
		...
		МассивДок = ИИТ_ВспомогательныеФункцииСервер.НайтиДокИзмененияДоговора(Объект.Ссылка);
...
Процедура АЛФ_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	ПолеВвода_АЛФ.ПутьКДанным = "Объект.ИИТ_ДоговорУчета";
     ПолеВвода_АЛФ.УстановитьДействие("ПриИзменении", "ДоговорУчетаПриИзменении"); //49_2 СмирновРВ
