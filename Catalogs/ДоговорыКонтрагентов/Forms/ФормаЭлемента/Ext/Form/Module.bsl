...
&НаКлиенте
Процедура ИИТ_АналитикаПоОбъектамСтроительстваБюджетПриИзменении(Элемент) //58 
     //ИИТ_АналитикаПоОбъектамСтроительстваБюджетПриИзмененииНаСервере();
КонецПроцедуры  

&НаСервере
Процедура ИИТ_АналитикаПоОбъектамСтроительстваБюджетПриИзмененииНаСервере() //58
	//Для Каждого СтрокаТЧ Из Объект.ИИТ_АналитикаПоОбъектамСтроительства Цикл
	//	Структура = Новый Структура;
	//	Структура.Вставить("Договор"			,Объект.Ссылка);
	//	Структура.Вставить("ОбъектСтроительства",СтрокаТЧ.ОбъектСтроительства);
	//	Структура.Вставить("Этап"				,СтрокаТЧ.ЭтапСтроительства);
	//	Структура.Вставить("СуммаДоговора"		,СтрокаТЧ.Бюджет);
	//	 
	//	МассивДок = ИИТ_ВспомогательныеФункцииСервер.НайтиДокИзмененияДоговораПоЭтапу(Структура);
	//	
	//	Для Каждого СтрокаМассив Из МассивДок Цикл 
	//		ДокументОбъект = СтрокаМассив.Регистратор.ПолучитьОбъект(); 
	//		ДокументОбъект.СуммаДоговора = СтрокаТЧ.Бюджет; 
...
Процедура АЛФ_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
    ...
    ЭлементТаблицыФормы.УстановитьДействие("ПриИзменении", "ИИТ_АналитикаПоОбъектамСтроительстваБюджетПриИзменении");
	...
	ПолеВвода_АЛФ.Доступность = Ложь; //57
	
	Элементы.ИИТ_АналитикаПоОбъектамСтроительства.ТолькоПросмотр = Объект.ИИТ_ДоговорУчета; 
	
	КомандаФормы = Команды.Добавить("АС_УстановитьДУ"); //57 
	КомандаФормы.Заголовок = "Установить ДУ";
	КомандаФормы.Действие  = "АС_УстановитьДУ"; 
	
	КнопкаФормы = Элементы.Вставить("ФормаАС_УстановитьДУ", Тип("КнопкаФормы"), Элементы.ГруппаШапкаПравая); //57 
	КнопкаФормы.Заголовок 	= "Установить ДУ";
	КнопкаФормы.ИмяКоманды 	= "АС_УстановитьДУ";
	КнопкаФормы.Вид 		= ВидКнопкиФормы.ОбычнаяКнопка; 
...
&НаКлиенте 
Процедура АС_УстановитьДУ(Элемент) //57
	
	ЗаполнитьАналитикаПоОбъектамСтроительстваСервер();
	
	АС_УстановитьДУСервер();
...
&НаСервере 
Процедура АС_УстановитьДУСервер()
	текДоговорУчета = Не Объект.ИИТ_ДоговорУчета; 
	...
	Для Каждого СтрокаТЧ Из Объект.ИИТ_АналитикаПоОбъектамСтроительства Цикл
		...
		Структура.Вставить("Этап"				,СтрокаТЧ.ЭтапСтроительства);
		Структура.Вставить("СуммаДоговора"		,СтрокаТЧ.Бюджет);
		Если текДоговорУчета Тогда //Вариант Не Учет -> Учет 
			Док = ИИТ_ВспомогательныеФункцииСервер.СоздатьИзмененияДоговора(Структура);
		Иначе   //Вариант Учет -> Не Учет
			МассивДок = ИИТ_ВспомогательныеФункцииСервер.НайтиПоследнийДокИзмененияДоговораПоЭтапу(Структура);
	...
	Объект.ИИТ_ДоговорУчета =  текДоговорУчета;
