...
Процедура АЛФ_ПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	ПолеВвода_АЛФ = Элементы.Вставить("АС_ПроектБюджета", Тип("ПолеФормы"), Элементы.ГруппаРеквизиты,);
	ПолеВвода_АЛФ.Заголовок 	= "Проект Бюджета";
	ПолеВвода_АЛФ.Вид 			= ВидПоляФормы.ПолеВвода;	
	ПолеВвода_АЛФ.ПутьКДанным 	= "Объект.АС_ПроектБюджета";
	ПолеВвода_АЛФ.УстановитьДействие("ПриИзменении", "АС_ПроектБюджетаПриИзменении"); //49_2
...
//49_2
&НаКлиенте
Процедура АС_ПроектБюджетаПриИзменении(Элемент) 
	Если ЕстьДвиженияБюджетирования() Тогда 
		ТекстВопроса = НСтр("ru = 'По Проекту Бюджета есть записи регистре Бюджетирование,
		|изменить записи регистра?'");
		Режим = РежимДиалогаВопрос.ДаНет; 
		Оповещение = Новый ОписаниеОповещения("ИзменитьОбъектСтроительстваБюджета", ЭтаФорма,); 
		ПоказатьВопрос(Оповещение, ТекстВопроса, Режим, 0);
	КонецЕсли;	
КонецПроцедуры

&НаСервере
Функция ЕстьДвиженияБюджетирования()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	АС_БюджетРасходов_2_6.Регистратор КАК Регистратор
		|ИЗ
		|	РегистрНакопления.АС_БюджетРасходов_2_6 КАК АС_БюджетРасходов_2_6
		|ГДЕ
		|	АС_БюджетРасходов_2_6.ОбъектСтроительства = &ОбъектСтроительства";
	
	Запрос.УстановитьПараметр("ОбъектСтроительства", Объект.АС_ПроектБюджета);
...
&НаСервере
Процедура ИзменитьОбъектСтроительстваБюджетаСервер() 
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	АС_БюджетРасходов_2_6.Регистратор КАК Док
		|ИЗ
		|	РегистрНакопления.АС_БюджетРасходов_2_6 КАК АС_БюджетРасходов_2_6
		|ГДЕ
		|	АС_БюджетРасходов_2_6.ОбъектСтроительства = &ОбъектСтроительства";
	...
	Пока Выборка.Следующий() Цикл 	
		Набор = РегистрыНакопления.АС_БюджетРасходов_2_6.СоздатьНаборЗаписей();
		Набор.Отбор.Регистратор.Установить(Выборка.Док);
		Набор.Прочитать();		
		Для Каждого Запись Из Набор Цикл 
            Запись.ОбъектСтроительства = Объект.АС_ПроектБюджета;
		КонецЦикла;  
		Набор.Записать();
	КонецЦикла;
...
&НаКлиенте
Процедура ИзменитьОбъектСтроительстваБюджета(Результат, Параметры) Экспорт 
	Если Результат = КодВозвратаДиалога.Да Тогда
	  ИзменитьОбъектСтроительстваБюджетаСервер();
	КонецЕсли;  
КонецПроцедуры	
//49_2
