...
Процедура АЛФ_ПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	ПолеВвода_АЛФ.УстановитьДействие("ПриИзменении", "АС_ПроектБюджетаПриИзменении");
	
	 ТекущийПроектБюджета = Объект.АС_ПроектБюджета; //57
...
Процедура АС_ПроектБюджетаПриИзменении(Элемент)
	Структура = Новый Структура;
	Структура.Вставить("Этап"		, ТекущийПроектБюджета); //ПроектБюджета 
	Структура.Вставить("Вид"		, "ПроектБюджета");
	Структура.Вставить("НовыйЭтап"	, Объект.Проект); //НовыйПроектБюджета
	Структура.Вставить("Договор"	, Неопределено);

	ИИТ_ВспомогательныеФункцииКлиент.ПроектБюджетаПриИзменении(Структура);
