...
КонецФункции
   ...
   &НаКлиенте
Процедура ИИТ_Ознакомиться(Команда)

	Для Каждого ВыделеннаяСтрока Из Элементы.ТекущиеОповещенияСписок.ВыделенныеСтроки Цикл
		ДанныеСтроки = Элементы.ТекущиеОповещенияСписок.ДанныеСтроки(ВыделеннаяСтрока);
		ИИТ_ОзнакомитьсяНаСервере(ДанныеСтроки.Ссылка, ДанныеСтроки.Пользователь);
	КонецЦикла;
...
&НаСервере
Процедура ИИТ_ОзнакомитьсяНаСервере(Заявка, Ответственный)
