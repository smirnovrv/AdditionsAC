...
#КонецУдаления
	...
	Если РасшифровкаПлатежа.Количество() > 0 Тогда
		...
		Для Каждого СтрокаРасшифровки ИЗ РасшифровкаПлатежа Цикл
			...
			Если АС_ОИФ Тогда 
				Движения.АС_СменаФормы.Записывать = Истина;  
				
				ДвижениеАС = Движения.АС_СменаФормы.ДобавитьПриход();  
				ДвижениеАС.Период 				= Дата;
				ДвижениеАС.Контрагент 			= Контрагент;
				ДвижениеАС.МОЛ 					= АС_МОЛ;   
				ДвижениеАС.ОбъектСтроительсва 	= ИИТ_ВспомогательныеФункцииСервер.МаксимальныйУровеньСправочника(СтрокаРасшифровки.Проект);
				ДвижениеАС.Сумма            	= СтрокаРасшифровки.СуммаПлатежа;
				ДвижениеАС.Комиссия         	= СтрокаРасшифровки.СуммаПлатежа * (АС_КомиссияМОЛПроцент/100);
				ДвижениеАС.СуммаКПолучению  	= СтрокаРасшифровки.СуммаПлатежа - (СтрокаРасшифровки.СуммаПлатежа * (АС_КомиссияМОЛПроцент/100)); 
			КонецЕсли;
