...
Процедура АЛФ_ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Иначе
		...
		Для Каждого СтрокаТЗ Из Товары Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТЗ.ИнвентарныйНомер) Тогда
				...
				Характеристика = ИИТ_ВспомогательныеФункцииСервер.ПолучитьХарактеристикуНоменклатурыСправочник(СтрокаТЗ);
	...
	//Себестоимость	
	НаборЗаписейРег = РегистрыНакопления.ИИТ_ОстаткиТМЦ.СоздатьНаборЗаписей();
	НаборЗаписейРег.Отбор.Регистратор.Установить(Ссылка);
	НаборЗаписейРег.Прочитать();
	
	Для Каждого Запись Из НаборЗаписейРег Цикл 
		
		Структура = Новый Структура;
		Структура.Вставить("Правило"	,Справочники.АС_ПравилаФормированияКлючейАналитики.НайтиПоКоду("000000001"));
		Структура.Вставить("Аналитика1"	,Запись.Номенклатура);
		Структура.Вставить("Аналитика2"	,Запись.Характеристика);
		Структура.Вставить("Аналитика3"	,Запись.Склад);
		Структура.Вставить("Код"		,Запись.Номенклатура.Код);
		
		ИИТ_ВспомогательныеФункцииСервер.СформироватьСебестоимость(Структура);
...
Процедура АЛФ_ОбработкаУдаленияПроведения(Отказ)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	АС_КлючАналитикиСебестоимостиДокументыДвижения.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.АС_КлючАналитикиСебестоимости.ДокументыДвижения КАК АС_КлючАналитикиСебестоимостиДокументыДвижения
		|ГДЕ
		|	АС_КлючАналитикиСебестоимостиДокументыДвижения.Ссылка.Проведен";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Для Каждого СтрокаТЧ Из ДокументОбъект.ДокументыДвижения Цикл
			Если СтрокаТЧ.Регистратор = Ссылка Тогда 
				 ДокументОбъект.ДокументыДвижения.Удалить(СтрокаТЧ);
			КонецЕсли;	
		КонецЦикла;	
	КонецЦикла;
