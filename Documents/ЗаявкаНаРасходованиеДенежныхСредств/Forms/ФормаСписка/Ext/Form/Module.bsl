...
Процедура АЛФ_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	КнопкаФормы.Вид 		= ВидКнопкиФормы.КнопкаКоманднойПанели;
	//КнопкаФормы.Доступность = ИИТ_ВспомогательныеФункцииСервер.РольДоступна_("АС_ИзменениеЭтапаВСогласованныхЗаявка"); врем 56
...
&НаСервере
Функция ЭтоДоговорУчета(Договор)
	 Возврат Договор.ИИТ_ДоговорУчета;
КонецФункции
...
Процедура ПриЗавершенииВыбораПроекта(Результат, ДополнительныеПараметры)  Экспорт 
	...
	Если ЗначениеЗаполнено(Результат) Тогда 
		тЭтап = ?(ЗначениеЗаполнено(ТекущиеДанные.ЭтапПроцесса), ТекущиеДанные.ЭтапПроцесса, ТекущиеДанные.ЭтапСтроительства);
		Структура = Новый Структура;
		Структура.Вставить("Этап"			, тЭтап); 
		Структура.Вставить("Вид"			, "Этап");
		Структура.Вставить("НовыйЭтап"		, Результат);
		Структура.Вставить("Договор"		, ТекущиеДанные.ДоговорКонтрагента);
		Структура.Вставить("ЗаявкаНаОплату"	, ТекущиеДанные.Ссылка);
