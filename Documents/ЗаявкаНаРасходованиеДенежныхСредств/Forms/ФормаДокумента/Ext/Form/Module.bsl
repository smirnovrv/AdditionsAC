...
&НаКлиентеНаСервереБезКонтекста
&ИзменениеИКонтроль("ДоговораСПоставками_УправлениеФормой")
Процедура АЛФ_ДоговораСПоставками_УправлениеФормой(Форма)
...
#Удаление
	Элементы.ТаблицаДвиженийЦФО.Видимость 			= ЭтоОплатаПоПоставкам;
	Элементы.ТаблицаДвиженийПроект.Видимость 		= ЭтоОплатаПоПоставкам;
#КонецУдаления
#Вставка
	Элементы.ТаблицаДвиженийЦФО.Видимость 			= Истина;
	Элементы.ТаблицаДвиженийПроект.Видимость 		= Истина;
#КонецВставки
...
Процедура АЛФ_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПереключательРасшифровки = ?(Объект.ЛицевыеСчетаСотрудников.Количество() > 0
	И (УчетЗарплатыИКадровВоВнешнейПрограмме И ВедетсяУчетРасчетовПоЗарплатеСводно), 1, 0);	
