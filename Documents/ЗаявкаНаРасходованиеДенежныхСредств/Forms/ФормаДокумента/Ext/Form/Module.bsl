...
Процедура ВывестиОстатокПоДоговору() 
	Сумма = 0;       
	
	Структура = Новый Структура; 
	Структура.Вставить("ОбъектСтроительства", Объект.ИИТ_ПроектСогласования.АС_ПроектБюджета);
	Структура.Вставить("Этап"				, Объект.Проект);
	Структура.Вставить("Договор"			, Объект.ДоговорКонтрагента);
	
	Если Объект.ДоговорКонтрагента.ИИТ_ДоговорУчета  Тогда
		СуммаОстаток = ПолучитьОстатокОплатыПоДоговру(Структура);
		Элементы.НадписьОстатокОплаты.Заголовок = "Остаток по договору: " + Формат(СуммаОстаток, "ЧЦ=15; ЧДЦ=2") + " руб.";
	Иначе     
		СуммаОстаток = ПолучитьОстатокОплаты(Структура);
		Элементы.НадписьОстатокОплаты.Заголовок = "Остаток прочий: " + Формат(СуммаОстаток, "ЧЦ=15; ЧДЦ=2") + " руб.";
	КонецЕсли;
КонецПроцедуры 

Функция ПолучитьОстатокОплатыПоДоговру(Структура)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	АС_БюджетРасходов_2_6Остатки.СуммаДоговораОстаток КАК СуммаДоговораОстаток
		|ИЗ
		|	РегистрНакопления.АС_БюджетРасходов_2_6.Остатки(
		|			&НаДату,
		|			Договор = &Договор
		|				И ОбъектСтроительства = &ОбъектСтроительства
		|				И Этап = &Этап) КАК АС_БюджетРасходов_2_6Остатки";
	
	Запрос.УстановитьПараметр("Договор"				, Структура.Договор);
	Запрос.УстановитьПараметр("НаДату"				, ТекущаяДата());
	Запрос.УстановитьПараметр("ОбъектСтроительства"	, Структура.ОбъектСтроительства);
	Запрос.УстановитьПараметр("Этап"				, Структура.Этап);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СуммаОстаток = ВыборкаДетальныеЗаписи.СуммаДоговораОстаток;
	КонецЦикла;
...
Функция ПолучитьОстатокОплаты(Структура)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	АС_БюджетРасходов_2_6Остатки.СуммаБюджетаОстаток КАК СуммаБюджетаОстаток
		|ИЗ
		|	РегистрНакопления.АС_БюджетРасходов_2_6.Остатки(
		|			&НаДату,
		|			ОбъектСтроительства = &ОбъектСтроительства
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СуммаОстаток = ВыборкаДетальныеЗаписи.СуммаБюджетаОстаток; 
	КонецЦикла;	
...
&НаКлиенте
Процедура АС_ОбновитьОстаток(Команда) 
	ВывестиОстатокПоДоговору();	
КонецПроцедуры	
...
Процедура УстановитьДоступностьПоТекущемуСогласующему()
	...
	Если ЭтаФорма.СтатусОбъекта = Перечисления.СостоянияСогласования.Черновик Тогда
	...
	Если ТекущийСогласующий = ПараметрыСеанса.ТекущийПользователь ИЛИ ПользователиЯвляеютсяЗаместителями(ТекущийСогласующий, ПараметрыСеанса.ТекущийПользователь) Тогда
		...
		Элементы.Комментарий.ТолькоПросмотр							= Ложь;
		Элементы.Комментарий.Доступность 							= Истина;
		Элементы.ГруппаКомментарийОтветственныйГор.ТолькоПросмотр 	= Ложь;
		Элементы.ГруппаКомментарийОтветственный.ТолькоПросмотр 		= Ложь; 
		Элементы.ПраваяЧасть.ТолькоПросмотр 						= Ложь;
...
&НаКлиенте
Процедура АС_ОИФПриИзменении(Элемент)

	Элементы.ГруппаАС_МОЛ.Видимость = Объект.АС_ОИФ; 
	
	Если  Объект.АС_ОИФ Тогда 
		АС_УстановитьСтатьюДДС();
	КонецЕсли;
...
Процедура АС_УстановитьСтатьюДДС()  
	
	Объект.ДвиженияОперации[0].СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("00-000065");
...
Процедура АЛФ_ПослеЗаписиПосле(ПараметрыЗаписи)
	...
	АЛФ_УстановитьВидимость(); //57
	Если Не АС_ТекущийЭтап = Объект.Проект Тогда
		Структура = Новый Структура;
		Структура.Вставить("Этап"			, АС_ТекущийЭтап); 
		Структура.Вставить("Вид"			, "Этап");
		Структура.Вставить("НовыйЭтап"		, Объект.Проект); 
		Структура.Вставить("Договор"		, Объект.ДоговорКонтрагента);
		Структура.Вставить("ЗаявкаНаОплату"	, Объект.Ссылка); 
		
		ИИТ_ВспомогательныеФункцииКлиент.ПроектБюджетаПриИзменении(Структура); //57 		
	КонецЕсли;
...
Процедура АЛФ_ПриОткрытииПосле(Отказ)    
	...
	АЛФ_УстановитьВидимостьКлиент();
	//ПодключитьОбработчикОжидания("АЛФ_УстановитьВидимостьКлиент", 1, Ложь); //52
...
&НаКлиенте
Процедура АЛФ_ДоговорКонтрагентаПриИзмененииПосле(Элемент)
	ВывестиОстатокПоДоговору(); 
КонецПроцедуры

&НаСервере
Процедура АЛФ_УправлениеФормойСервер()
	//Команды формы
	...
	КомандаФормы = Команды.Добавить("АС_ОбновитьОстаток");  
	КомандаФормы.Заголовок = "Обновить остаток";
	КомандаФормы.Действие  = "АС_ОбновитьОстаток";  
	...
	ПолеВвода_АЛФ = Элементы.Добавить("АС_ОИФ", Тип("ПолеФормы"), Элементы.Группа_ДатаНомер);
	ПолеВвода_АЛФ.Заголовок 	= "ОИФ";
	ПолеВвода_АЛФ.Вид 			= ВидПоляФормы.ПолеФлажка;	
	ПолеВвода_АЛФ.ПутьКДанным 	= "Объект.АС_ОИФ"; 
	ПолеВвода_АЛФ.УстановитьДействие("ПриИзменении", "АС_ОИФПриИзменении"); 
	ПолеВвода_АЛФ.Видимость = РольДоступна("АС_Кассир");  
	
	ПолеВвода_АЛФ = Элементы.Добавить("АС_ИсходныйДУ", Тип("ПолеФормы"), Элементы.Группа_ДатаНомер); //57
	ПолеВвода_АЛФ.Вид 			= ВидПоляФормы.ПолеФлажка;	
	ПолеВвода_АЛФ.ПутьКДанным 	= "Объект.АС_ИсходныйДУ"; 
	...
	Элементы.Переместить(Элементы.ДоговорКонтрагента, Элементы.ЛеваяЧасть);
	...
	НоваяГруппа = Элементы.Добавить("ГруппаАС_МОЛ", Тип("ГруппаФормы"), Элементы.ЛеваяЧасть);
	НоваяГруппа.Вид 				= ВидГруппыФормы.ОбычнаяГруппа;
	НоваяГруппа.Группировка 		= ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	НоваяГруппа.Отображение 		= ОтображениеОбычнойГруппы.Нет;
	НоваяГруппа.ОтображатьЗаголовок = Ложь;  
	...
	Декорация_АЛФ 		= Элементы.Добавить("НадписьОстатокОплаты", Тип("ДекорацияФормы"), НоваяГруппа_1);
	Декорация_Алф.Вид 	= ВидДекорацииФормы.Надпись; 
	Декорация_Алф.Шрифт = Новый Шрифт(Декорация_АЛФ.Шрифт,,,Истина);
	
	КнопкаФормы = Элементы.Вставить("ФормаАС_ОбновитьОстаток", Тип("КнопкаФормы"), НоваяГруппа_1);  
	КнопкаФормы.Заголовок 	= "Обновить остаток";
	КнопкаФормы.ИмяКоманды 	= "АС_ОбновитьОстаток";
	КнопкаФормы.Вид 		= ВидКнопкиФормы.ОбычнаяКнопка;  
	...
	ПолеВвода_АЛФ 							= Элементы.Добавить("КомментарийККонтрагенту", Тип("ПолеФормы"), Элементы.ОплатаКонтрагенту);
	ПолеВвода_АЛФ.Вид 						= ВидПоляФормы.ПолеВвода;
	ПолеВвода_АЛФ.ПутьКДанным 				= "КомментарийККонтрагенту";
	ПолеВвода_АЛФ.АвтоМаксимальнаяВысота 	= Ложь; 
	ПолеВвода_АЛФ.АвтоМаксимальнаяШирина 	= Ложь; 
	ПолеВвода_АЛФ.МногострочныйРежим 		= Истина;
	ПолеВвода_АЛФ.Доступность            	= Ложь;
	
	Элементы.Переместить(НоваяГруппа, Элементы.ПраваяЧасть);	
	Элементы.Переместить(Элементы.СтрокаПлатежнаяПозиция, Элементы.ГруппаОсновноеРеквизиты);

	НоваяГруппа_11 = Элементы.Добавить("НоваяГруппа_11", Тип("ГруппаФормы"), Элементы.ГруппаОсновное);
	НоваяГруппа_11.Вид = ВидГруппыФормы.ОбычнаяГруппа;
	НоваяГруппа_11.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	НоваяГруппа_11.Отображение = ОтображениеОбычнойГруппы.Нет;
	НоваяГруппа_11.ОтображатьЗаголовок = Ложь;  
		  
	Элементы.Переместить(Элементы.НазначениеПлатежа	, НоваяГруппа_11); 
	Элементы.Переместить(Элементы.ЗаказыПоставщику	, НоваяГруппа_11);
	
	Попытка
		УстановитьКомментарийККонтрагенту();
	Исключение
	КонецПопытки;
	
	 АС_ТекущийЭтап = Объект.Проект; //57
	 
КонецПроцедуры 

&НаСервере
Процедура УстановитьКомментарийККонтрагенту() 
	
	КомментарийККонтрагенту = Объект.Контрагент.Комментарий;

КонецПроцедуры

&НаКлиенте
Процедура АЛФ_УстановитьВидимостьКлиент() 
  АЛФ_УстановитьВидимость();
КонецПроцедуры
...
Процедура АЛФ_УстановитьВидимость() 
	...
	Элементы.ГруппаФормаОплаты.Видимость 		= Ложь;
	Элементы.ГруппаОчередностьПлатежа.Видимость = Ложь; 
	Элементы.ГруппаОчередностьПлатежа.Видимость = Ложь;
	Элементы.ПлатежСКонвертацией.Видимость 		= Ложь;
...
&НаКлиенте
Процедура АЛФ_ПередЗаписьюПосле(Отказ, ПараметрыЗаписи)
	Если Параметры.Ключ.Пустая() Тогда 
		Объект.АС_ИсходныйДУ = ПолучитьДоговорУчета();  
	КонецЕсли;
	Если Не Объект.СуммаДокумента = Объект.СуммаДокументаУпр Тогда //58 
		ИзменитьСуммуЗавкиВДокументах(); 		
	КонецЕсли;	
КонецПроцедуры 

&НаСервере
Процедура ИзменитьСуммуЗавкиВДокументах(); //58 
	Структура = Новый Структура;
	Структура.Вставить("Договор"			,Объект.ДоговорКонтрагента);
	Структура.Вставить("ОбъектСтроительства",Объект.Проект.АС_ПроектБюджета);
	Структура.Вставить("Этап"				,Объект.Проект);
	Структура.Вставить("СуммаДоговора"		,Объект.СуммаДокумента);
	Структура.Вставить("ЗаявкаНаОплату"		,Объект.Ссылка);
	
	МассивДок = ИИТ_ВспомогательныеФункцииСервер.НайтиДокИзмененияЭтапаПоЗаявке(Структура);
	
	Для Каждого СтрокаМассив Из МассивДок Цикл 
		ДокументОбъект = СтрокаМассив.Регистратор.ПолучитьОбъект(); 
		ДокументОбъект.СуммаДоговора = Объект.СуммаДокумента; 
		ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
	КонецЦикла;
	
	Объект.АС_ИсходныйДУ = Ложь;
КонецПроцедуры	

&НаСервере
Функция ПолучитьДоговорУчета()   
	Возврат Объект.ДоговорКонтрагента.ИИТ_ДоговорУчета; 
КонецФункции
