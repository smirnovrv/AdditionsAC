...
Процедура АЛФ_ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		Комментарий 			= ДанныеЗаполнения.Комментарий; //СмирновРВ
...
#Вставка     
	...
	Структура.Вставить("Договор"			,ДоговорКонтрагента);
	Структура.Вставить("ОбъектСтроительства",Проект.АС_ПроектБюджета);
	Структура.Вставить("Этап"				,Проект);
	Структура.Вставить("ЗаявкаНаОплату"		,Ссылка);
	...
	Для Каждого СтрокаМассив Из МассивДок Цикл
		уДокумент = СтрокаМассив.Ссылка.ПолучитьОбъект();
		Отбор = Новый Структура;
		Отбор.Вставить("ДокументСсылка", Структура.ЗаявкаНаОплату);
		нСтроки = уДокумент.КорректируемыеДокументы.НайтиСтроки(Отбор); 
		Для Каждого стр Из нСтроки Цикл  
			уДокумент.КорректируемыеДокументы.Удалить(стр);
		КонецЦикла;
		уДокумент.Записать(РежимЗаписиДокумента.Проведение);
	КонецЦикла;
	// 
#КонецВставки
КонецПроцедуры
...
Процедура АЛФ_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)  
	...
	Если Ссылка.ДоговорКонтрагента.ПометкаУдаления Тогда 
...
//+ИИТ_Аксеньюшкин
	...
	Для Каждого СтрокаДанных Из ДвиженияОперации Цикл				
		...
		Движение.ОбъектСтроительства 	= ТекЭтап.Родитель.АС_ПроектБюджета;
		...
		Движения.АС_БюджетРасходов_2_6.Записывать = Истина;
		Движение = Движения.АС_БюджетРасходов_2_6.ДобавитьРасход(); 	
		Движение.Период                 = Дата;
		Движение.Регистратор            = Ссылка;
		Движение.ОбъектСтроительства 	= ТекЭтап.Родитель.АС_ПроектБюджета;
		Движение.Этап                   = ТекЭтап;
		Движение.Договор                = ДоговорКонтрагента; 
		Движение.Контрагент         	= ДоговорКонтрагента.Владелец;
				
		Если ИИТ_ВспомогательныеФункцииСервер.ЕстьДокументИзменениеДоговора(Ссылка) Тогда //57
			Если АС_ИсходныйДУ Тогда 
				Движение.СуммаДоговора 		= СтрокаДанных.Сумма;
			Иначе
				Движение.СуммаБюджета 		= СтрокаДанных.Сумма;
			КонецЕсли;
		Иначе 
			Если ДоговорКонтрагента.ИИТ_ДоговорУчета Тогда 
				Движение.СуммаДоговора 		= СтрокаДанных.Сумма;
			Иначе
				Движение.СуммаБюджета 		= СтрокаДанных.Сумма;
			КонецЕсли;
...
#КонецУдаления	
	...
	КонецЕсли;
	Если АС_ОИФ Тогда
		ПроверяемыеРеквизиты.Добавить("АС_КомиссияМОЛПроцент"); 
		ПроверяемыеРеквизиты.Добавить("АС_МОЛ");
	Иначе 
		Если Не ПроверяемыеРеквизиты.Найти("АС_КомиссияМОЛПроцент") = Неопределено Тогда 
			ПроверяемыеРеквизиты.Удалить(ПроверяемыеРеквизиты.Найти("АС_КомиссияМОЛПроцент"));
		КонецЕсли;
		Если Не ПроверяемыеРеквизиты.Найти("АС_МОЛ") = Неопределено Тогда
			ПроверяемыеРеквизиты.Удалить(ПроверяемыеРеквизиты.Найти("АС_МОЛ"));
		КонецЕсли; 		
	КонецЕсли;  
	Если АС_ОИФ Тогда
		Если  НЕ ДвиженияОперации[0].СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("00-000065") Тогда  
			Сообщить(Строка(Ссылка) + " Не верно выбрана Статья ДДС");
			Отказ = Истина;
		КонецЕсли;
	Иначе
		Если  ДвиженияОперации[0].СтатьяДвиженияДенежныхСредств = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("00-000065") Тогда 
			Сообщить(Строка(Ссылка) + " Не верно выбрана Статья ДДС");
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
